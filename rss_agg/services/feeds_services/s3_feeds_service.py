...
def boto3_session_factory(
    aws_default_region: Annotated[domain.AwsRegion, Inject(config="aws_default_region")],
    aws_access_key_id: Annotated[domain.AwsAccessKey, Inject(config="aws_access_key_id")],
    aws_secret_access_key: Annotated[domain.AwsSecretAccessKey, Inject(config="aws_secret_access_key")],
) -> Session:
    return Session(
...
def s3_client_factory(
    session: Session,
    s3_endpoint: Annotated[domain.S3Endpoint, Inject(config="s3_endpoint")],
) -> BaseClient:
...
class S3FeedsService(FeedsService):
    def __init__(
        ...
        bucket_name: Annotated[domain.BucketName, Inject(config="feeds_bucket_name")],
        object_name: Annotated[domain.ObjectName, Inject(config="feeds_object_name")],
